-- Final Supabase Security Migration - Fix All Remaining Issues
...
-- These are the tables identified by the linter

-- Add policies for any remaining tables that need them
-- (Note: Some policies may already exist from previous migrations)

-- Ensure all critical tables have proper RLS policies
CREATE POLICY IF NOT EXISTS "Users can view their own referral codes" ON public.referral_codes
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can create their own referral codes" ON public.referral_codes
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Service can manage referral codes" ON public.referral_codes
  FOR ALL USING (auth.role() = 'service_role'::text);

-- Add policies for rate_limits table if it exists
CREATE POLICY IF NOT EXISTS "Users can view their own rate limits" ON public.rate_limits
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Service can manage rate limits" ON public.rate_limits
  FOR ALL USING (auth.role() = 'service_role'::text);

-- Add policies for user_analytics table if it exists
CREATE POLICY IF NOT EXISTS "Users can view their own analytics" ON public.user_analytics
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own analytics" ON public.user_analytics
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own analytics" ON public.user_analytics
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Service can manage analytics" ON public.user_analytics
  FOR ALL USING (auth.role() = 'service_role'::text);
